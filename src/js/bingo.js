...
function updateNumCards() {
  const numCardsElement = /** @type {HTMLInputElement} */ (
    document.getElementById('numCards')
  );
...
function makePdf() {
  ...
  while (bingoCards.size < numCards) {
    const shuffledWords = /** @type {string[]} */ (
      fisherYatesShuffle([...wordList])
    );
...
function updateGenerateButton() {
  ...
  if (wordList.length < 24) {
    reasons.push(
      `Add more words (need at least 24, currently ${wordList.length})`,
    );
  }
