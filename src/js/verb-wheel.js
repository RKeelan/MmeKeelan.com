...
// DOM Elements
const pronounReelElement = /** @type {HTMLElement} */ (
  document.getElementById('pronoun-reel')
);
...
function drawWheelSegments() {
  ...
  verbs.forEach((verb, index) => {
    ...
    const textAngle =
      startAngle + anglePerSegment + Math.PI - anglePerSegment / 2;
...
spinButton.addEventListener('click', () => {
  ...
  const rotations =
    MIN_ROTATIONS + Math.floor(Math.random() * MAX_ADDITIONAL_ROTATIONS);
