...
// DOM Elements
const subjectReelElement = /** @type {HTMLElement} */ (
  document.getElementById('subject-reel')
);
if (!subjectReelElement) throw new Error('subject-reel element not found');

const verbReelElement = /** @type {HTMLElement} */ (
  document.getElementById('verb-reel')
);
if (!verbReelElement) throw new Error('verb-reel element not found');

const objectReelElement = /** @type {HTMLElement} */ (
  document.getElementById('object-reel')
);
if (!objectReelElement) throw new Error('object-reel element not found');

const languageToggle = /** @type {HTMLButtonElement} */ (
...
const spinButton = /** @type {HTMLButtonElement} */ (
...
function getPronounArray(isEnglish) {
  return isEnglish
    ? englishPronouns
    : englishPronouns.map((pronoun) => translations[pronoun]);
}
...
spinButton.addEventListener('click', () => {
  ...
  const randomEnglishVerb =
    englishVerbs[Math.floor(Math.random() * englishVerbs.length)];
  const randomVerb = isEnglishMode
    ? randomEnglishVerb
    : translations[randomEnglishVerb];
  ...
  const randomObject = isEnglishMode
    ? randomEnglishObject
    : translations[randomEnglishObject];
  ...
  const totalObjectsRemaining = Object.values(workingVerbObjectMap).reduce(
