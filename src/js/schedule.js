...
const createTimeBlockList = (data) => {
  ...
  for (let minutes = startTime; minutes <= endTime; minutes += INTERVAL) {
    ...
    DAYS_OF_WEEK.forEach((day) => {
      ...
      allBlocksForDay.forEach(
        /** @type {function(any): void} */ (block) => {
          const [start, end] = block.Time.split(' - ');
          const startMinutes = timeToMinutes(start);
          const endMinutes = timeToMinutes(end);

          if (startMinutes === minutes) {
            /** @type {any} */ (timeBlock)[day] = {
              Block: block.Block,
              Duration: endMinutes - startMinutes,
            };
            totalMinutesMap[block.Block] =
              (totalMinutesMap[block.Block] || 0) + (endMinutes - startMinutes);
          }
        },
      );
    });
...
const createTable = (data) => {
  ...
  for (let minutes = currentTime; minutes <= endTime; minutes += INTERVAL) {
    ...
    const invariantBlock =
      data.Invariants &&
      data.Invariants.find(
        /** @type {function(any): boolean} */ (block) => {
    ...
    if (invariantBlock) {
      ...
      invariantCell.style.backgroundColor =
        (data.Colors && data.Colors.Invariants) || 'lightgray';
      ...
      DAYS_OF_WEEK.forEach((day) => {
        ...
        if (timeBlock[day]) {
          ...
          dayCell.style.backgroundColor =
            (data.Colors && data.Colors[timeBlock[day].Block]) || 'white';
...
const renderTables = () => {
  const yamlTextarea = /** @type {HTMLTextAreaElement} */ (
    document.getElementById('yaml-textarea')
  );
  ...
  try {
    ...
    errorMessage.textContent =
      'Invalid YAML: ' + /** @type {Error} */ (e).message;
    const container = document.getElementById('tables-container');
    if (!container) return;
    container.innerHTML = '';
